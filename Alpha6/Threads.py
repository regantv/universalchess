...
class ButtonCheck(QThread):
    ...
    def checkButtons(self):
        if self.robot.get_digital_in(2) == True:
            doneSignal.emit(0)
        if self.robot.get_digital_in(3) == True:
            doneSignal.emit(1)
        if self.robot.get_digital_in(4) == True:
            doneSignal.emit(2)
        ...
        if self.mode ==1:
            ...
            if len(moveList)>9:
                ...
                if len(moves)==0:
                    ...
                    if (moveList[1]-1==moveList[0]) and (moveList[2]-2==moveList[0]) and (moveList[3]-3==moveList[0]):
                        print("castling")
                        for i in moves:
                            if self.playerBoard.is_castling(i):
                                ...
                                self.robot.set_digital_out(5, False)
                                break
                    elif (moveList[0]+2==moveList[1]) and (moveList[2]-1==moveList[1]) and (moveList[3]-2==moveList[1]):
                        print("castling")
                        for i in moves:
                            if self.playerBoard.is_castling(i):
                                data =  '1'+str(self.player)+str(i.from_square)+"-"+str(i.to_square)
...
class RoboWorker(QThread):
    ...
    def Promotion(self):
        ...
        if figure in ('Q','K','P','R','N','B'):
            position[0]+= 0.036* dataStructers.figuresDropWhitePos[key][0]
            position[1]+= 0.036* dataStructers.figuresDropWhitePos[key][1]
            position[2] = current[2]
        else:
            position[0]+= 0.036* dataStructers.figuresDropBlackPos[key][0]
            position[1]+= 0.036* dataStructers.figuresDropBlackPos[key][1]
            position[2] = current[2]
...
if __name__ == '__main__':
   ...
   #     robot = urx.Robot("192.168.1.20", use_rt=True)
    ...
    pose = [1.8060463666915894, -1.7235668341266077, -0.8052786032306116, -2.277634922658102,-1.5196850935565394,3.566532850265503]
